...
const Cards = ({ item, data, setData }) => {
    ...
    const updateStory = async (e) => {
        e.preventDefault()
        await axios.put(`${baseUrl}/api/v1/story/${item?._id}`, {
            title: titleInputRef.current?.value,
            text: bodyInputRef.current?.value
        }).then((response) => {
            toast.success(response?.data?.message)

            let updatedItem = data.find((element) => { return element._id === item._id })

            updatedItem.title = titleInputRef.current?.value
            updatedItem.text = bodyInputRef.current?.value
    ...
    const DeleteStory = async () => {
        await axios.delete(`${baseUrl}/api/v1/story/${item?._id}`).then((response) => {
            toast.success(response?.data?.message)
            setData(
                data?.filter((i) => {
            ...
            <div className="p-2 lg:w-1/2 md:w-1/2 w-full">
                <div className="h-full flex flex-col items-center border-gray-300 border p-4 rounded-lg">
                    <div className="flex-grow">
                        <h2 className="text-gray-800 text-xl font-medium">{item?.title}</h2>
                        <p className="text-gray-600 text-lg text-left">{item?.text}</p>
                    </div>
                                                ...
                                                <div className="flex flex-wrap -m-2">
                                                    <div className="p-2 w-full">
                                                        <div className="relative">
                                                            ...
                                                            <input defaultValue={item?.title} id="titleInput" ref={titleInputRef} type="text" className="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-blue-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
                                                        </div>
                                                    </div>
                                                    <div className="p-2 w-full">
                                                        <div className="relative">
                                                            <label htmlFor="bodyInput" className="leading-7 text-sm text-gray-600">Decription</label>
                                                            <textarea defaultValue={item?.text} id="bodyInput" ref={bodyInputRef} className="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"></textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
